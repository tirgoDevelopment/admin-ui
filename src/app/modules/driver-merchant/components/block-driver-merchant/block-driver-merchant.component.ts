...
@Component({
  selector: 'app-block-driver-merchant',
  templateUrl: './block-driver-merchant.component.html',
  styleUrls: ['./block-driver-merchant.component.scss'],
...
export class BlockDriverMerchantComponent {
  ...
  constructor(
    ...
    private _driverMerChantService: DriverMerchantService,
  ...
  submit() {
    if (this.form.valid) {
      this._driverMerChantService.block(this.form.get('id').value, this.form.get('block_reason').value,).subscribe(res => {
        if (res.success) {
